...
"""Generic benchmark dashboard synthesis.

This module merges machine-readable benchmark artifacts into a single dashboard
payload that can be consumed by gates, CI, and humans.

The implementation is stage-agnostic; stage- or gate-specific runners should only
provide the input artifact paths and the desired defaults for output locations.
"""
...
DEFAULT_TEXT_SUMMARY_LABEL = "Benchmark dashboard"
...
def format_text_summary(
    report: dict[str, Any],
    *,
    summary_label: str = DEFAULT_TEXT_SUMMARY_LABEL,
) -> str:
    lines = [f"{summary_label} — {report['stage']} / {report['gate']}", "=" * 60]
...
def build_benchmark_dashboard(
    ...
    *,
    ...
    default_gate: str = DEFAULT_GATE,
    title: str = DEFAULT_TITLE,
    text_summary_label: str = DEFAULT_TEXT_SUMMARY_LABEL,
) -> dict[str, Any]:
    ...
    report = {
        "title": title,
        "stage": coverage_gap.get("stage", audit_report.get("stage", default_stage)),
        "gate": coverage_gap.get("gate", audit_report.get("gate", default_gate)),
        "sources": {
    ...
    report["text_summary"] = format_text_summary(report, summary_label=text_summary_label)
    return report


def _cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build a benchmark dashboard")
    ...
    parser.add_argument("--default-gate", default=DEFAULT_GATE)
    parser.add_argument("--title", default=DEFAULT_TITLE)
    parser.add_argument("--summary-label", default=DEFAULT_TEXT_SUMMARY_LABEL)
    args = parser.parse_args(argv)

    report = build_benchmark_dashboard(
        ...
        title=args.title,
        text_summary_label=args.summary_label,
    )
