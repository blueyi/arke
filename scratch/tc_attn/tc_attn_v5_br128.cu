// TC flash-attention v5 — BR=128, 8 warps (256 threads).
//
// Same architecture as v4b but doubled query tile size:
// - BR=128 (vs 64): each block processes 128 query rows
// - NWARPS=8 (vs 4): warp i owns rows [i*16 .. i*16+15]
// - Qsh: [128][64] = 16384 B (16K)
// - KV double-buffer: 2 × (BC*D*2) = 2 × 16K = 32K
// - Total smem: 16K + 32K = 48K → 2 blocks/SM (48K × 2 = 96K < 100K)
//
// Grid: ceil(S/128) × (B*H)
//
// Key insight (same as v4b): P stays in registers as matrix_a fragments.
// No Psh needed. Softmax is register-resident.
...
extern "C" __global__ void tc_flash_attn_v5_br128(
    ...
    int B, int H, int S, float scale)
{
    ...
    // Shared memory: 48 KB (NO Psh!)
    extern __shared__ char smem[];
    half* Qsh_    = (half*) smem;                        // 16384 B (128*64*2)
    ...
    // Total: 16384 + 16384 + 16384 = 49152 B = 48 KB
    ...
    // Load Q — 128 rows × 64 cols = 8192 half elements, 256 threads
    ...
    for (int t = 0; t < ntiles; t++) {
        ...
        if (t + 1 < ntiles) {
            ...
            int per_thread = (BC * D) / blockDim.x;  // 4096/256=16 elements
