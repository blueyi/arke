// TC flash-attention v4b — v4 + register-resident P (no Psh).
//
// Eliminates Psh entirely: softmax outputs stay in registers, directly
// constructed into matrix_a fragments for PV matmul.
// This saves 8K smem AND one __syncthreads per tile iteration.
//
// Smem: Qsh(8K) + KV_slot0(16K) + KV_slot1(16K) = 40 KB → 2 blocks/SM
// (40K × 2 = 80K < 100K; could potentially fit 2 blocks)
//
...
extern "C" __global__ void tc_flash_attn_v4b(
