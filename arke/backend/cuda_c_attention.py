...
def emit_cuda_c_flash_attention_splitk(
    graph: IRGraph, chip: str = "sm_86", nwarp: int = 8,
) -> CudaCKernel:
    \"\"\"Emit a split-K FlashAttention-2 kernel: O = softmax(Q@K^T/sqrt(D)) @ V.

    Parallelization (vs the warp-per-row base kernel):
      - One block per (bh, query-row). Grid.x = S so there are S× more blocks
        per (batch,head), which fills the machine at large S where the
        warp-per-row kernel starves (S/BR blocks, each warp serially scanning
        all S keys — an O(S) latency chain).
      - NWARP warps in the block SPLIT the K dimension: warp w scans keys
        w, w+NWARP, w+2*NWARP, ... Each warp runs its own online softmax over
        its K-subset, so the serial K-scan length drops from S to S/NWARP.
      - Within a warp the 32 lanes split the head dim (d = lane + 32*t), so the
        Q·K dot is a warp-shuffle reduction and P·V accumulates lane-local.
      - Warps combine partial (m, l, acc) via the FlashAttention rescale:
        gm = max_w m_w; gl = sum_w l_w*exp(m_w-gm);
        acc[d] = sum_w acc_w[d]*exp(m_w-gm); O = acc/gl. This is exact online
        softmax, numerically identical to a single-pass scan.
...
// Q,K,V,O: [{B},{H},{S},{D}] row-major. One block per (bh, query-row).
// {NWARP} warps split the K dimension; 32 lanes split the head dim.
#include <cuda_runtime.h>
#include <math.h>
#include <float.h>
...
{{
    int bh = blockIdx.y;
    int q_row = blockIdx.x;           // one query row per block
    int warp = threadIdx.x >> 5;      // which K-split this warp owns
    int lane = threadIdx.x & 31;      // lane within the warp
    ...
    for (int t = 0; t < DPL; t++) {{
        int d = lane + 32 * t;
        q_reg[t] = (d < D) ? Qbh[q_row * D + d] : 0.0f;
        acc[t] = 0.0f;
    }}
    {c_type} m_i = -FLT_MAX;
    {c_type} l_i = 0.0f;

    // Split-K: warp w scans keys w, w+NWARP, w+2*NWARP, ...
    for (int k_row = warp; k_row < S; k_row += NWARP) {{
        {c_type} s_partial = 0.0f;
        #pragma unroll
        for (int t = 0; t < DPL; t++) {{
            int d = lane + 32 * t;
            if (d < D) s_partial += q_reg[t] * Kbh[k_row * D + d];
        }}
        #pragma unroll
        for (int off = 16; off > 0; off >>= 1)
            s_partial += __shfl_down_sync(0xffffffff, s_partial, off);
        ...
        for (int t = 0; t < DPL; t++) {{
            int d = lane + 32 * t;
            if (d < D) acc[t] = acc[t] * corr + p * Vbh[k_row * D + d];
        }}
        m_i = m_new;
    }}

    // ── cross-warp combine (FlashAttention rescale) ──
    __shared__ {c_type} sm[NWARP];        // per-warp running max
    __shared__ {c_type} sl[NWARP];        // per-warp running denom
    __shared__ {c_type} sacc[NWARP][HEAD_DIM];
    if (lane == 0) {{ sm[warp] = m_i; sl[warp] = l_i; }}
    #pragma unroll
    for (int t = 0; t < DPL; t++) {{
        int d = lane + 32 * t;
        if (d < D) sacc[warp][d] = acc[t];
    }}
    __syncthreads();

    // Global max + denom over all warps (redundant across threads, NWARP small).
    {c_type} gm = -FLT_MAX;
    #pragma unroll
    for (int w = 0; w < NWARP; w++) gm = fmaxf(gm, sm[w]);
    {c_type} gl = 0.0f;
    #pragma unroll
    for (int w = 0; w < NWARP; w++) gl += sl[w] * expf(sm[w] - gm);

    // Warp 0 writes the output row; its lanes own the head-dim slices.
    if (warp == 0) {{
