...
def _gen_tiled_matmul_ir_doublebuf(
    kernel_name: str, M: int, K: int, N: int,
    BM: int, BN: int, BK: int, num_tiles: int,
) -> str:
    """Generate LLVM IR for double-buffered, bounds-check-free tiled matmul.

    Uses 2 shmem buffers for A and B to overlap global loads with compute.
    BM=64, BN=64, BK=16, TM=4, TN=4, Block=(16,16)=256 threads.
    Requires M%BM==0, N%BN==0, K%BK==0 (no bounds checks needed).
    """
    TM, TN = 4, 4
    BLOCK_X, BLOCK_Y = 16, 16
    ...
    # Flat base for loading (each thread loads 4 elements)
    ln("  %flat_base = mul i32 %lin_tid, 4")

    # Precompute shmem load indices (invariant across tiles)
    for i in range(4):
        ln(f"  %sa{i}_flat = add i32 %flat_base, {i}")
        ln(f"  %sa{i}_srow = lshr i32 %sa{i}_flat, 4")  # / 16
        ln(f"  %sa{i}_scol = and i32 %sa{i}_flat, 15")   # % 16
        ln(f"  %sa{i}_grow = add i32 %row_base, %sa{i}_srow")

    for i in range(4):
        ln(f"  %sb{i}_flat = add i32 %flat_base, {i}")
        ln(f"  %sb{i}_srow = lshr i32 %sb{i}_flat, 6")  # / 64
        ln(f"  %sb{i}_scol = and i32 %sb{i}_flat, 63")   # % 64
        ln(f"  %sb{i}_gcol = add i32 %col_base, %sb{i}_scol")

    # ---- Preload tile 0 into buffer 0 ----
    ln("  ; -- Preload tile 0 into buffer 0 --")
    for i in range(4):
        sfx = f"pre_a{i}"
        # Global A index: A[sa_grow, 0*BK + sa_scol] = A[sa_grow][sa_scol]
        ln(f"  %{sfx}_gidx = mul i32 %sa{i}_grow, {K}")
        ln(f"  %{sfx}_gidx2 = add i32 %{sfx}_gidx, %sa{i}_scol")
        ln(f"  %{sfx}_gep = getelementptr float, float addrspace(1)* %A, i32 %{sfx}_gidx2")
        ln(f"  %{sfx}_val = load float, float addrspace(1)* %{sfx}_gep")
        ln(f"  %{sfx}_ptr = getelementptr [2 x [{BM} x [{BK} x float]]], [2 x [{BM} x [{BK} x float]]] addrspace(3)* @shmem_A, i32 0, i32 0, i32 %sa{i}_srow, i32 %sa{i}_scol")
        ln(f"  store float %{sfx}_val, float addrspace(3)* %{sfx}_ptr")

    for i in range(4):
        sfx = f"pre_b{i}"
        # Global B index: B[sb_srow, sb_gcol]
        ln(f"  %{sfx}_gidx = mul i32 %sb{i}_srow, {N}")
        ln(f"  %{sfx}_gidx2 = add i32 %{sfx}_gidx, %sb{i}_gcol")
        ln(f"  %{sfx}_gep = getelementptr float, float addrspace(1)* %B, i32 %{sfx}_gidx2")
        ln(f"  %{sfx}_val = load float, float addrspace(1)* %{sfx}_gep")
        ln(f"  %{sfx}_ptr = getelementptr [2 x [{BK} x [{BN} x float]]], [2 x [{BK} x [{BN} x float]]] addrspace(3)* @shmem_B, i32 0, i32 0, i32 %sb{i}_srow, i32 %sb{i}_scol")
        ln(f"  store float %{sfx}_val, float addrspace(3)* %{sfx}_ptr")
    ...
    # ---- Tile body: load next tile + compute current ----
    ln("tile_body:")
    ...
    for i in range(4):
        sfx = f"nla{i}"
        ln(f"  %{sfx}_gcol = add i32 %next_tile_bk, %sa{i}_scol")
        ln(f"  %{sfx}_gidx = mul i32 %sa{i}_grow, {K}")
        ln(f"  %{sfx}_gidx2 = add i32 %{sfx}_gidx, %{sfx}_gcol")
        ln(f"  %{sfx}_gep = getelementptr float, float addrspace(1)* %A, i32 %{sfx}_gidx2")
        ln(f"  %{sfx}_val = load float, float addrspace(1)* %{sfx}_gep")
        ln(f"  %{sfx}_ptr = getelementptr [2 x [{BM} x [{BK} x float]]], [2 x [{BM} x [{BK} x float]]] addrspace(3)* @shmem_A, i32 0, i32 %buf_next, i32 %sa{i}_srow, i32 %sa{i}_scol")
        ln(f"  store float %{sfx}_val, float addrspace(3)* %{sfx}_ptr")

    for i in range(4):
        sfx = f"nlb{i}"
        ln(f"  %{sfx}_grow = add i32 %next_tile_bk, %sb{i}_srow")
        ln(f"  %{sfx}_gidx = mul i32 %{sfx}_grow, {N}")
        ln(f"  %{sfx}_gidx2 = add i32 %{sfx}_gidx, %sb{i}_gcol")
        ln(f"  %{sfx}_gep = getelementptr float, float addrspace(1)* %B, i32 %{sfx}_gidx2")
        ln(f"  %{sfx}_val = load float, float addrspace(1)* %{sfx}_gep")
        ln(f"  %{sfx}_ptr = getelementptr [2 x [{BK} x [{BN} x float]]], [2 x [{BK} x [{BN} x float]]] addrspace(3)* @shmem_B, i32 0, i32 %buf_next, i32 %sb{i}_srow, i32 %sb{i}_scol")
        ln(f"  store float %{sfx}_val, float addrspace(3)* %{sfx}_ptr")

    ln("  br label %compute")
    ln("")

    # ---- Compute on current buffer (k-loop) ----
    ln("compute:")
    ln("  br label %k_loop_header")
    ln("")
    ...
    # ---- k-loop body ----
    ln("k_loop_body:")
    # Load 4 values from shmem_A[buf][ty*4+r][ki]
    for r in range(TM):
    ...
    # Load 4 values from shmem_B[buf][ki][tx*4+c]
    for c in range(TN):
    ...
    # ---- k-loop exit: barrier, then latch ----
    ln("k_loop_exit:")
    ln("  call void @llvm.nvvm.barrier0()")
    ln("  br label %tile_loop_latch")
    ln("")
    ...
    # ---- Store 4x4 results (no bounds check) ----
    ln("store_result:")
    for r in range(TM):
        for c in range(TN):
            ln(f"  %final_r{r}c{c} = phi float [ %acc_r{r}c{c}, %tile_loop_header ]")
    ...
    for r in range(TM):
        for c in range(TN):
            row_r = f"%row0" if r == 0 else f"%row_r{r}"
            col_c = f"%col0" if c == 0 else f"%col_c{c}"
            if r > 0 and c == 0:
