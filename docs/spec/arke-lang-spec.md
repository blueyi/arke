...
Specifies which operations should be fused together. `ops` may name explicit SemanticIR node ops, or a compact registered fused op's logical inner ops when the surface is represented as one canonical op (for example `silu_and_mul` as `silu` + `mul`, or `gelu_and_mul` as `gelu` + `mul`).
