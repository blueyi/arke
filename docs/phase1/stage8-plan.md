...
| D8-X1 | **Extensibility Demo A — new operator onboarding** (≤400 LOC, 1 SKILL, 1 audit, BL1 evidence; rename legacy `swiglu` benchmark op → `silu_and_mul` + legacy `geglu` benchmark op → `gelu_and_mul`, then register true-fused OT3 op `swiglu_packed`; see "Tier 1 Extensibility Acceptance" above and `docs/roadmap/plan.md` HARNESS-3) | P0 | M | ✅ (2026-06-06; commit chain `3917493` C1 → `911bd9d` C2 → `4c4b118` C3 → `8cea142` C4 → `b2e8bd1` C5 on `feat/op-count-ssot`; catalog 45 → 46 ops, OT3 7 → 8; SKILL at `skills/swiglu-packed-fusion/SKILL.md`; audit at `docs/benchmark/audit/swiglu_packed_baseline_audit_2026-05-30.md`; BL1 evidence at `benchmarks/results/phase1/stage8/extensibility/bl1_new_op.csv`) |
| D8-X2 | **Extensibility Demo B — new baseline runner onboarding** (≤200 LOC, BaselineRunner protocol doc, BL1+BL3 evidence) | P0 | M | ✅ (2026-06-25; `MaxAutotuneRunner` `benchmarks/baselines/max_autotune.py` 141 LOC wraps `torch.compile(mode=max-autotune)`, registered in `bench_l1.py`; BL1 evidence `benchmarks/results/phase1/stage8/extensibility/baseline_max_autotune/`; 4 unit tests `tests/test_max_autotune_runner.py`) |
| D8-X3 | Document `BaselineRunner` ABC + onboarding playbook in `docs/architecture/arke-harness.md` new § | P0 | S | ✅ (2026-06-25; `arke-harness.md` §9.1 "Onboarding a new BaselineRunner" — 6-member ABC table + 5-step playbook + Demo B worked example) |
...
| D7-E1.5 | **Hit G8[4] target**: rerun `bench_l3 --model gpt2 --seq-len 128,256,512 --runs 20 --warmup 10`; require ratio_vs_eager ≥0.95 at all three seq lens; commit fresh CSV+summary.json under `benchmarks/results/phase1/stage8/track4/l3/` and update M1/M4 status. **Outcome (2026-05-17): satisfied by E1.6 same-day rerun → `summary.json: g8_gpt2_pass=true`, all three seq_lens ≥ 1.024×.** ⚠️ **KNOWN-FAIL re-opened (2026-06-25, Leon-acked "标记为失败用例,后续再解决"):** a fresh rerun on the same RTX 3060 (warmup=10/runs=20, `dynamic=True`, cache_size_limit=64) reproduced seq=128 0.970× ✅ / seq=512 1.136× ✅ but **seq=256 0.880× (multi-seq) / 0.801× (isolated single-seq) ❌** — NOT the prior eviction artifact (isolated re-measurement is *worse*). Real cause: vanilla torch.compile genuinely loses to eager at GPT-2 seq=256 on 6GB SM8.6 (launch+guard overhead dominates small matmuls). The 2026-05-17 1.070× reading was not reproducible 2026-06-25; treat seq=256 as an **evidence-backed known-fail** (not relaxed, not deleted) pending later resolution. Evidence: `benchmarks/results/phase1/stage8/track4/gpt2_real_2026-06-25/` (commit `8a25cd3`). Gate口径 (min/geomean/per-seq) pending Leon D3. | P0 | S | 🚧 known-fail (seq=256) |
| D7-E1.4 | **Arke→`torch.compile` bridge MVP (G8[4b] critical path, scope-guarded)**: implement `arke/integration/torch_bridge.py` — a **single-file** shim that registers Arke-generated Triton kernels as `torch.library.custom_op` so `torch.compile`'d models (transformers GPT-2) can dispatch into Arke kernels as atomic ops. Ship 2 ops end-to-end (rmsnorm + matmul) with: (a) numerical correctness vs eager (rtol/atol per benchmark-fairness.md); (b) `register_fake` abstract impl for dynamo trace; (c) inference-only (no autograd backward); (d) bridge-invocation-evidence — torch profiler / dynamo log showing ≥1 Arke kernel invocation per forward on GPT-2 critical path. **This bridge is a transient Substrate artifact, not a product capability** — see "D7-E1.4 scope guardrails" below. | P0 | L | ✅ (2026-06-25; `arke/integration/torch_bridge.py` + `__init__.py`; `arke::matmul`+`arke::rmsnorm` custom_op w/ register_fake; 5 unit tests `tests/test_torch_bridge.py`; **invocation evidence: arke::matmul fires 48×/GPT-2 forward on Conv1D critical path, top-1 match vs eager (329==329), logits max-diff 0.125** — `benchmarks/results/phase1/stage8/track4/bridge_2026-06-25/`; doc §3.0.3 marked shipped) |
...
| D7-E2 | LLaMA-2 7B integration + bench_l3 runner | P0 | L | 🚧 LLaMA-family validated via D4=L2 (TinyLlama-1.1B, ungated LLaMA-arch): bench_l3 `--arch llama` generic causal-LM loader; correctness 100%, geomean 1.239× eager (all seq ≥1.12×), no OOM @4.5GB. Evidence `benchmarks/results/phase1/stage8/track4/tinyllama_2026-06-25/`. Full gated LLaMA-2 7B deferred to larger GPU (6GB blocked: gated DL + missing bnb/accelerate). |
| D7-E3.0 | **DS-V2 16B reachability probe** (S size, Leon-approved 2026-05-17, Q2=a): before committing to D7-E3 full integration, run a 1-day probe — load DS-V2 16B in 4bit quant on RTX 3060 6GB, forward at seq=512 batch=1, measure peak VRAM + latency floor. Two outcomes: (i) fits & forward completes → confirm D7-E3 as G8[6] critical path; (ii) OOMs even at smallest config → escalate to Leon for decision (audit-only G8[6] vs reschedule to bigger GPU). Deliverable: `benchmarks/results/phase1/stage8/track4/dsv2_probe_YYYY-MM-DD/probe.md` with VRAM trace, latency, decision recommendation. **Outcome (2026-06-25, outcome (ii)):** DS-V2 16B 4-bit ≈ 8+ GB > 6.44 GB total VRAM → **OOM, audit-only** (evidence `benchmarks/results/phase1/stage8/track4/multimodel_probe_2026-06-25/probe.md`). Also: `bitsandbytes`+`accelerate` missing in venv (4-bit load blocked). Per no-relaxation: recorded audit-only with VRAM evidence, not excluded. | P0 | S | ✅ (audit-only) |
