...
|:---------|:------|:--------|:-----------------|
| `arke/integration/torch_bridge.py` | Phase 1 only | BL6 G8[4b] endpoint validation — show Arke kernels can be embedded in `torch.compile`'d HF transformers forward graphs via `torch.library.custom_op` | **✅ SHIPPED 2026-06-25.** Single file; 2 ops (`arke::rmsnorm` + `arke::matmul`); inference-only (`register_fake` only, no autograd backward); **not** exported from `arke.__init__`; **not** part of the Façade contract; explicit opt-in via `register_arke_ops()`. Verified: arke::matmul fires **48×/forward** on GPT-2's Conv1D critical path with top-1 token match vs eager (logits max-abs-diff 0.125). Full guardrails: `docs/phase1/stage8-plan.md`. |
