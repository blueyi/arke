...
| S8 | G8 | BL5 inherit + BL6×L3 (GPT-2+LLaMA-2+DS-V2) | L1+L2+L3 | Agent Autonomy | 🚧 D7-E1.1 diagnosis ✅ — GPT-2 torch.compile: seq=128/512 already ≥1.0× eager; only seq=256 fails (0.865×) due to dynamic-shape recompile thrash. M1 critical path = D7-E1.6 cache_size_limit + dynamic=True. |
