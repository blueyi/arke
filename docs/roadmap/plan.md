...
| S7 | G7 | BL5×L1+L2 | L1+L2 | Lang & IR v0.1.0 (high-level IR ready for Phase-3 MLIR consumption) | 🚧 13/14 (G7.8d honest-gap; autotune + L2 Triton baseline follow-up open) |
...
**Current G7 evidence status (2026-05-16):** Implementation/test slices for Lang, IR, MLIR skeleton, examples, backend-agnostic StrategyIR, and non-regression are green. After the 2026-05-14 rope-fp32 (commit `82b635b`) + gated odd-N typed-unsupported (commit `160ebf4`) + 2026-05-15 G7.8b coverage closure (commit `13d42e6`) fixes, canonical Track 6 evidence passes G7.8c (correctness 863/863 ok across all baselines) and G7.8b (BL5 L1 coverage 45/45 ops, 685/685 shapes). G7.8d has been re-calibrated under the **Same-Backend Triton Fairness** rule locked on 2026-05-16 (commits `c366116` docs + `3c89a23` scoring): the perf denominator is now `min(latency_us)` over Triton-only baselines per `(layer, op, shape_tag)`, with ε=0.03 tolerance and audit-only handling when no Triton reference is available. Under this honest ruler, current Track 6 PERF_ALL yields L1 weighted_score=0.3000 (per-OT pass rates: ot0_1=54.1%, ot2=30.6%, ot3=36.4%, ot4=0/0 due to missing Triton attention ref data) and L2 fusions=0 evaluable (no Triton baseline collection yet). Audit-only rows: `perf_oracle_unavailable_triton=438`. The gap is **real distance vs. ladder-fastest Triton kernels**, not a measurement artifact — follow-up work (L1 Triton-template autotune rollout to remaining 20 templates; L2 Triton fusion baseline collection) is required to reach the locked 0.97 per-group floor / 0.95 weighted threshold. See [docs/phase1/stage7-completion-summary.md](../phase1/stage7-completion-summary.md) for the full gap analysis.
